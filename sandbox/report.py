...
from threat_score import generate


# Helper function to format UTC datetime in ISO 8601 format for OpenSearch
...
class Resources:
    ...
    @staticmethod
    def getResources(raw, driver_session, report_id, scan_url, first_seen):
        mapping = []
        resource_master = {}
        for request_id in raw:
            if 'response' in raw[request_id]:
                try:
                    ...
                    if 'body' in response_body:
                        ...
                        mapping.append({'sha256': sha256, 'url': raw[request_id]['request']['url'], "mime_type": raw[request_id]['response']['mimeType']})
                        resource_master[sha256] = {"report_id": report_id, "resource": response_body['body'],
                                                   "sha256": sha256, "mime_type": raw[request_id]['response']['mimeType'],
                                                   "request_id": request_id, "submission_url": scan_url,
                                                   "resource_url": raw[request_id]['response']['url'], 
                                                   "first_seen_utc": first_seen, "last_seen_utc": first_seen}
                except:
                    pass
...
class Formatting:
    ...
    def clean_data(report):
        ...
        if 'server' in report:
            servers = [server for server in report['server']]
        return report
...
class Enrichment:
    ...
    def __init__(self, config=None):
        self.ip_asn = geoip2.database.Reader(r'/app/geoIP/GeoLite2-ASN.mmdb')
        self.ip_country = geoip2.database.Reader(r'/app/geoIP/GeoLite2-Country.mmdb')
        self.country_cache = {}
        self.asn_cache = {}
        
        # Create a custom TLDExtract instance with in-memory cache
        self.extractor = tldextract.TLDExtract(cache_dir=False)
        
        # Store config if provided, otherwise will be loaded on demand
        if config:
            self.config = config
    ...
    def scanMeta(report):
        ...
        meta = {'request_count': 0, 'script_count': 0, 'domain_count': 0, 'submission_url': report['submission_url'], 'submission_utc': report['submission_utc'], 'risk_score': score}
    ...
    def get_dns_info(self, domain):
        # Check if DNS lookups are enabled in config
        if not hasattr(self, 'config'):
            # Import config if not already available
            import os
            self.config = {
                'dns': os.getenv('dns', 'NONE')
            }
        
        # Return empty dict if DNS lookups are disabled
        if self.config['dns'] == 'NONE' or self.config['dns'] == 'False':
            return {}
    ...
    def domain_extract(self, url):
        dirty = urlparse(url).netloc
        clean = self.extractor(dirty)
