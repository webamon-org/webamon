...
class Helper:
        ...
        for doc in docs:
            if not self.id_exists(doc, 'resources'):
                try:
                    ...
                    docs[doc]['domains'] = []
                    docs[doc]['notes'] = []
                    docs[doc]['whitelisted'] = None
                    docs[doc]['blacklisted'] = None
    ...
    def save_report(self, report):
        # Prepare the OpenSearch connection details
        base_url = f"{self.protocol}://{self.host}:{self.port}"
        index_url = f"{base_url}/scans/_doc/{report['report_id']}"  # Native OpenSearch URL for indexing
        headers = {"Content-Type": "application/json"}

        # Remove unnecessary fields based on configuration
        resources = report.pop('resource_master', False)
        if not self.save_dom:
            self.logger.debug("NOT SAVING DOM")
            report.pop('dom', False)

        # Save the report using a POST request
        self.logger.debug("SAVING REPORT TO OPENSEARCH")
        response = requests.post(index_url, headers=headers, json=report, verify=False, auth=self.auth)

        # Handle response
        if response.status_code not in [200, 201]:
            self.logger.critical(f"Failed to save report: {response.status_code} - {response.text}")
            return False

        # Save resources if required
            ...
            if resources:
                ...
                if bulk_data:
                    response = requests.post(
                        f'{self.config["elastic_base"]}/_bulk',
                        headers=headers,
                        data=bulk_data,
                        verify=False,
                        auth=self.auth
...
class Domains(Helper):
    ...
    def update(self, domain_record):
        ...
        domain_record['last_seen'] = datetime.utcnow().strftime("%Y-%m-%d")
        domain_record['last_seen_utc'] = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")

        exists = self.id_exists(domain_b64, index)
        if not exists:
            domain_record['first_seen_utc'] = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
...
class Servers(Helper):
    ...
    def update(self, server_record):
        ...
        server_record['last_seen'] = datetime.utcnow().strftime("%Y-%m-%d")
        server_record['last_seen_utc'] = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")

        exists = self.id_exists(ip_b64, index)
        if not exists:
            server_record['first_seen_utc'] = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
